...
import re
import sys
# The output is too big for the command terminal, so I'm going to 
# print everything to 'output.txt' located in the same directory.
...
def fields_for_records(form, fields):
    ...
    for field in fields:
        print(indent(formId+'{0: <10}'.format("." + field.name) + " = record['fu_"+field.name.lower()+"']", 1), file=sys.stderr)
...
def main():
    ...
    # Right now I have the path defaulted to my csv folder 
    # where I'm keeping the lbd forms.
    data_dict_path = '/Users/s.emerson/Desktop/nacc_code/nacculator/nacc_lbd_excel'
    corrected_dict_path = './corrected/'
    header_file = 'lbd_fvp_ded.csv'
